...
LCL is a Python package for fitting latent-class conditional logit models. It runs the expectation-maximization algorithm on JAX, sharding the per-class M-steps across whatever accelerators it finds, and returns a results object that carries clustered robust standard errors, counterfactual predictions, and Delta-method willingness-to-pay distributions.

It is written for econometricians who routinely outgrow `mlogit`, `gmnl`, or `Apollo` on large panel datasets. Eighty thousand households, each with twenty choice occasions, is a comfortable working size on a single H200 — and the library has been put through Monte Carlo sweeps several times that.
...
- **`ConditionalLogit`** — a standard conditional logit, useful both as a baseline and as the inner kernel of the M-step.
...
A two-class model on a small synthetic panel. The full example, including counterfactual fares and value-of-time partitions, is in the [estimation tutorial](tutorials/estimation.md).
...
Numbers in parentheses are Delta-method standard errors on the population moments. The class-specific β's themselves are available on `results.em_res.structural_betas`.

## Roadmap

The estimator is stable and the results object covers the cases we encounter in our own work. Active development is on three fronts:

- **Model selection.** Blocked K-fold cross-validation is included but still marked experimental — expect refinements as we work through edge cases on highly unbalanced panels.
...
If there is a constraint, optimization routine, or post-estimation tool you would like to see, [open an issue](https://github.com/zeyveld/latent-class-conditional-logit/issues).
